...
(function () {
  ...
  ready(function () {
    ...
    function go(n, fromRemote){
      ...
      notes.innerHTML = getSlideNotes(n);
    ...
    if (bc) {
      bc.onmessage = function(e) {
        ...
        if (e.data.type === 'go' && typeof e.data.idx === 'number') {
          ...
          const note = slides[e.data.idx] && slides[e.data.idx].querySelector('.notes, aside.notes, .speaker-notes');
          if (note) note.innerHTML = e.data.notes || '';
          if (e.data.idx === idx) notes.innerHTML = e.data.notes || '';
        }
      };
    }
    ...
    function buildPresenterHTML(deckUrl, slideMeta, total, startIdx, channelName, currentTheme) {
      ...
      const notesStorageKey = 'html-ppt-presenter-notes:' + location.pathname;
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .pcard-notes .empty { color: #484f58; font-style: italic; }
	  ...
	  .notes-btn:hover {
	    background: rgba(240,136,62,.16);
	    border-color: #f0883e;
	    color: #f0883e;
	  }
	  #notes-body[contenteditable="true"] {
	    outline: none;
	    caret-color: #f0883e;
	  }
	  #notes-body[contenteditable="true"]:focus {
	    box-shadow: inset 0 0 0 1px rgba(240,136,62,.55);
	    background: rgba(240,136,62,.04);
	  }
	  #notes-body[contenteditable="true"] .empty {
  ...
  <div class="pcard pcard-notes" id="card-notes" style="--dot-color:#f0883e">
    <div class="pcard-head" data-drag>
      ...
      <span class="pcard-title">SPEAKER SCRIPT · 逐字稿</span>
	      <span class="notes-status" id="notes-status">可编辑 · 自动保存</span>
	      <span class="pcard-actions">
	        <button class="notes-btn" id="btn-notes-reset" type="button">还原本页</button>
	      </span>
    </div>
	    <div class="pcard-body" id="notes-body" contenteditable="true" spellcheck="false" aria-label="可编辑演讲稿"></div>
    <div class="pcard-resize" data-resize></div>
  </div>
...
(function(){
  ...
  function saveLayout() {
    ...
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(layout)); } catch(e) {}
	  ...
	  function writeSavedNotes(map) {
	    try { localStorage.setItem(NOTES_STORAGE_KEY, JSON.stringify(map)); } catch(e) {}
	  }
	  function originalNote(n) {
	    return originalSlideNotes[n] || '';
	  }
	  ...
	  function scheduleNotesSave() {
	    ...
	    clearTimeout(notesSaveTimer);
	    notesSaveTimer = setTimeout(saveCurrentNotes, 250);
	  }
  ...
  document.getElementById('btn-notes-reset').addEventListener('click', function(){
    ...
    renderNotes(idx);
    if (bc) bc.postMessage({ type: 'notes-update', idx: idx, notes: originalNote(idx) });
  });
  ...
  document.addEventListener('keydown', function(e){
    if (document.activeElement === notesBody || notesBody.contains(document.activeElement)) return;
    if (e.metaKey || e.ctrlKey || e.altKey) return;
    switch(e.key) {
      case 'ArrowRight': case 'PageDown': go(idx + 1); e.preventDefault(); break;
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    document.addEventListener('keydown', function (e) {
      if (e.metaKey||e.ctrlKey||e.altKey) return;
      switch (e.key) {
        case 'ArrowRight': case 'PageDown': case 'Enter': go(idx+1); e.preventDefault(); break;
